...
const InfosysMenu = ({ company, onBack, onOpenAIInterview }) => {
  ...
  const renderContent = () => {
    switch (activeMenu) {
      ...
      case 'Coding Round':
        return <CodingRound company={company} currentYear={currentYear} />;
      case 'Interview':
        ...
        <nav className="csr-menu">
          ...
          {isSyllabusExpanded && (
            <div className="csr-submenu">
              ...
              <div className={`csr-menu-item ${activeMenu === 'Coding Round' ? 'active' : ''}`} onClick={() => setActiveMenu('Coding Round')} style={{ paddingLeft: '40px', fontSize: '0.95em' }}>
                <span className="csr-menu-icon">💻</span> Coding Round
              </div>
