...
const AccentureMenu = ({ company, onBack, onOpenAIInterview }) => {
  ...
  const renderContent = () => {
    switch (activeMenu) {
      ...
      case 'Coding Assessment':
        return <CodingAssessment company={company} currentYear={currentYear} />;
      case 'Interview':
        ...
        <nav className="csr-menu">
          ...
          {isSyllabusExpanded && (
            <div className="csr-submenu">
              ...
              <div className={`csr-menu-item ${activeMenu === 'Coding Assessment' ? 'active' : ''}`} onClick={() => setActiveMenu('Coding Assessment')} style={{ paddingLeft: '40px', fontSize: '0.95em' }}>
                <span className="csr-menu-icon">💻</span> Coding Assessment
              </div>
