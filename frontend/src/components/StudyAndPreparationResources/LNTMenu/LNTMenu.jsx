...
const LNTMenu = ({ company, onBack, onOpenAIInterview }) => {
  ...
  const renderContent = () => {
    switch (activeMenu) {
      ...
      case 'Personal/Technical Interview':
        return <TechnicalInterview company={company} />;
      case 'Medical Examination':
        return <MedicalExamination company={company} />;
      default:
        return (
        ...
        <nav className="csr-menu">
          ...
          {isSyllabusExpanded && (
            <div className="csr-submenu">
              <div className={`csr-menu-item ${activeMenu === 'Online Test' ? 'active' : ''}`} onClick={() => setActiveMenu('Online Test')} style={{ paddingLeft: '40px', fontSize: '0.95em' }}>
                Online Test
              </div>
              <div className={`csr-menu-item ${activeMenu === 'Personal/Technical Interview' ? 'active' : ''}`} onClick={() => setActiveMenu('Personal/Technical Interview')} style={{ paddingLeft: '40px', fontSize: '0.95em' }}>
                Personal/Technical Interview
              </div>
              <div className={`csr-menu-item ${activeMenu === 'Medical Examination' ? 'active' : ''}`} onClick={() => setActiveMenu('Medical Examination')} style={{ paddingLeft: '40px', fontSize: '0.95em' }}>
                Medical Examination
              </div>
            </div>
          )}
