...
const CapgeminiMenu = ({ company, onBack, onOpenAIInterview }) => {
  ...
  const renderContent = () => {
    switch (activeMenu) {
      ...
      case 'Group Discussion':
        return <GroupDiscussion company={company} currentYear={currentYear} />;
      default:
        return (
        ...
        <nav className="csr-menu">
          ...
          {isSyllabusExpanded && (
            <div className="csr-submenu">
              ...
              <div className={`csr-menu-item ${activeMenu === 'Group Discussion' ? 'active' : ''}`} onClick={() => setActiveMenu('Group Discussion')} style={{ paddingLeft: '40px', fontSize: '0.95em' }}>
                <span className="csr-menu-icon">👥</span> Group Discussion
              </div>
            </div>
          )}
        </nav>
